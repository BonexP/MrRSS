...
const article = computed<Article | undefined>(() => store.articles.find(a => a.id === store.currentArticleId));
const showContent = ref(false);
const articleContent = ref('');
const isLoadingContent = ref(false);
const currentArticleId: Ref<number | null> = ref(null);
const defaultViewMode: Ref<ViewMode> = ref('original');
const pendingRenderAction: Ref<RenderAction> = ref(null);
const imageViewerSrc: Ref<string | null> = ref(null);
        ...
        <div v-else class="flex flex-col h-full bg-bg-primary">
            <div class="h-[44px] sm:h-[50px] px-3 sm:px-5 border-b border-border flex justify-between items-center bg-bg-primary shrink-0">
                <button @click="close" class="md:hidden flex items-center gap-1.5 sm:gap-2 text-text-secondary hover:text-text-primary text-sm sm:text-base">
                    <PhArrowLeft :size="18" class="sm:w-5 sm:h-5" /> <span class="hidden xs:inline">{{ t('back') }}</span>
                </button>
                <div class="flex gap-1 sm:gap-2 ml-auto">
                    <button @click="toggleContentView" class="action-btn" :title="showContent ? t('viewOriginal') : t('viewContent')">
                        <PhGlobe v-if="showContent" :size="18" class="sm:w-5 sm:h-5" />
                        <PhArticle v-else :size="18" class="sm:w-5 sm:h-5" />
                    </button>
                    <button @click="toggleRead" class="action-btn" :title="article.is_read ? t('markAsUnread') : t('markAsRead')">
                        <PhEnvelopeOpen v-if="article.is_read" :size="18" class="sm:w-5 sm:h-5" />
                        <PhEnvelope v-else :size="18" class="sm:w-5 sm:h-5" />
                    </button>
                    <button @click="toggleFavorite" :class="['action-btn', article.is_favorite ? 'text-yellow-500 hover:text-yellow-600' : 'hover:text-yellow-500']" :title="article.is_favorite ? t('removeFromFavorite') : t('addToFavorite')">
                        <PhStar :size="18" class="sm:w-5 sm:h-5" :weight="article.is_favorite ? 'fill' : 'regular'" />
                    </button>
                    <button @click="openOriginal" class="action-btn" :title="t('openInBrowser')">
