...
} = useArticleActions(t, defaultViewMode, () => store.fetchUnreadCounts());
...
onMounted(async () => {
    ...
    window.addEventListener('translation-settings-changed', onTranslationSettingsChanged as EventListener);
...
onBeforeUnmount(() => {
    cleanupTranslation();
    window.removeEventListener('translation-settings-changed', onTranslationSettingsChanged as EventListener);
    window.removeEventListener('default-view-mode-changed', onDefaultViewModeChanged as EventListener);
        ...
        <div class="p-2 sm:p-4 border-b border-border bg-bg-primary">
            <div class="flex items-center justify-between mb-2 sm:mb-3">
                ...
                <div class="flex items-center gap-1 sm:gap-2">
                    <button @click="markAllAsRead" class="text-text-secondary hover:text-text-primary hover:bg-bg-tertiary p-1 sm:p-1.5 rounded transition-colors" :title="t('markAllRead')">
                        <PhCheckCircle :size="20" class="sm:w-6 sm:h-6" />
                    </button>
                    <div class="relative">
                        <button @click="refreshArticles" class="text-text-secondary hover:text-text-primary hover:bg-bg-tertiary p-1 sm:p-1.5 rounded transition-colors" :title="t('refresh')">
            ...
            <div class="flex items-center gap-2">
                <div class="flex-1 flex items-center bg-bg-secondary border border-border rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 focus-within:border-accent transition-colors">
                    ...
                    <input type="text" v-model="searchQuery" :placeholder="t('search')" class="search-input bg-transparent border-none outline-none w-full ml-1.5 sm:ml-2 text-text-primary text-xs sm:text-sm" data-search-input>
                </div>
                <div class="relative">
                    <button @click="showFilterModal = true" class="filter-btn p-1.5 sm:p-2 rounded-lg transition-colors" :class="activeFilters.length > 0 ? 'filter-active' : ''" :title="t('filter')">
