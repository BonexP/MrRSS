...
onMounted(async () => {
    try {
        ...
        applySettings(data, (theme: string) => store.setTheme(theme as any));
    } catch (e) {
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-2 sm:p-4">
        <div class="bg-bg-primary w-full max-w-4xl h-full sm:h-[700px] sm:max-h-[90vh] flex flex-col rounded-none sm:rounded-2xl shadow-2xl border border-border overflow-hidden animate-fade-in">
            ...
            <div class="flex border-b border-border bg-bg-secondary shrink-0 overflow-x-auto scrollbar-hide">
                <button @click="activeTab = 'general'" :class="['tab-btn', activeTab === 'general' ? 'active' : '']">{{ t('general') }}</button>
                <button @click="activeTab = 'feeds'" :class="['tab-btn', activeTab === 'feeds' ? 'active' : '']">{{ t('feeds') }}</button>
                <button @click="activeTab = 'rules'" :class="['tab-btn', activeTab === 'rules' ? 'active' : '']">{{ t('rules') }}</button>
                <button @click="activeTab = 'shortcuts'" :class="['tab-btn', activeTab === 'shortcuts' ? 'active' : '']">{{ t('shortcuts') }}</button>
                <button @click="activeTab = 'about'" :class="['tab-btn', activeTab === 'about' ? 'active' : '']">{{ t('about') }}</button>
            </div>
