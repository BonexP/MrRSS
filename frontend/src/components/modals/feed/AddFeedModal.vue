...
<template>
    <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-bg-primary w-full max-w-md rounded-2xl shadow-2xl border border-border overflow-hidden animate-fade-in">
            ...
            <div class="p-6">
                <div class="mb-4">
                    <label class="block mb-1.5 font-semibold text-sm text-text-secondary">{{ t('title') }} ({{ t('optional') }})</label>
                    <input v-model="title" type="text" :placeholder="t('titlePlaceholder')" class="input-field">
                </div>
                <div class="mb-4">
                    <label class="block mb-1.5 font-semibold text-sm text-text-secondary">{{ t('rssUrl') }}</label>
                    <input v-model="url" type="text" :placeholder="t('rssUrlPlaceholder')" class="input-field">
                </div>
                <div class="mb-4">
                    <label class="block mb-1.5 font-semibold text-sm text-text-secondary">{{ t('categoryOptional') }}</label>
                    <input v-model="category" type="text" :placeholder="t('categoryPlaceholder')" class="input-field">
                </div>
            </div>
